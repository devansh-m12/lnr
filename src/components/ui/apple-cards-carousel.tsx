...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const handleCardClose = (index: number) => {
    if (carouselRef.current) {
      const cardWidth = isMobile() ? 230 : 100; // (md:w-96)
  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="relative z-10 flex h-48 w-40 flex-col items-start justify-start overflow-hidden rounded-3xl bg-gray-100 dark:bg-neutral-900 md:h-64 md:w-72"
      >
        ...
        <div className="relative z-40 p-4 md:p-6">
          ...
          <motion.p
            layoutId={layout ? `title-${card.title}` : undefined}
            className="mt-1 max-w-xs text-left font-sans text-base font-semibold text-white [text-wrap:balance] md:text-xl"
          >
            {card.title}
          </motion.p>
        </div>
